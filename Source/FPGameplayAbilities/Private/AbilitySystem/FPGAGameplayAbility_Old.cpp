...
#include "AbilitySystem/FPGAGameplayAbility_Old.h"
...
void UFPGAGameplayAbility_Old::OnCooldownFinished(const FGameplayEffectRemovalInfo& RemovalInfo)
{
...
// Cooldown //
//////////////
void UFPGAGameplayAbility_Old::FPGAApplyCooldown(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo)
{
...
bool UFPGAGameplayAbility_Old::HasCooldown()
{
...
bool UFPGAGameplayAbility_Old::CheckCooldown(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, FGameplayTagContainer* OptionalRelevantTags) const
{
...
float UFPGAGameplayAbility_Old::GetCooldownTimeRemaining(const FGameplayAbilityActorInfo* ActorInfo) const
{
	if (GetCooldownGameplayEffect())
	{
...
void UFPGAGameplayAbility_Old::GetCooldownTimeRemainingAndDuration(FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, float& TimeRemaining, float& CooldownDuration) const
{
	if (GetCooldownGameplayEffect())
	{
		if (const FActiveGameplayEffect* Effect = ActorInfo->AbilitySystemComponent->GetActiveGameplayEffect(CooldownEffectHandle))
...
void UFPGAGameplayAbility_Old::OnGameplayEffectApplied(UAbilitySystemComponent* Source, const FGameplayEffectSpec& SpecApplied, FActiveGameplayEffectHandle ActiveHandle)
{
	...
	if (Effect == AbilitySystem->GetGameplayEffectDefForHandle(ActiveHandle))
	{
		...
		AbilitySystem->OnGameplayEffectRemoved_InfoDelegate(ActiveHandle)->AddUObject(this, &UFPGAGameplayAbility_Old::OnCooldownFinished);
	}
}

void UFPGAGameplayAbility_Old::CommitExecute(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo)
{
...
bool UFPGAGameplayAbility_Old::CommitAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo)
{
	UGameplayEffect* Effect = GetCooldownGameplayEffect();
	if (Effect != nullptr)
	{
		...
		{
			MyDelHandle = AbilitySystem->OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(this, &UFPGAGameplayAbility_Old::OnGameplayEffectApplied);
		}
	}
...
bool UFPGAGameplayAbility_Old::CommitAbilityCooldown(
...
void UFPGAGameplayAbility_Old::BlockAndCancelTags()
{
...
void UFPGAGameplayAbility_Old::ActivateAbility(
	const FGameplayAbilitySpecHandle Handle,
	const FGameplayAbilityActorInfo* ActorInfo,
	const FGameplayAbilityActivationInfo ActivationInfo,
...
void UFPGAGameplayAbility_Old::EndAbility(
	const FGameplayAbilitySpecHandle Handle,
	const FGameplayAbilityActorInfo* ActorInfo,
	const FGameplayAbilityActivationInfo ActivationInfo,
...
bool UFPGAGameplayAbility_Old::CheckCost(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, FGameplayTagContainer* OptionalRelevantTags) const
{
...
void UFPGAGameplayAbility_Old::ApplyCost(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo) const
{
	UGameplayEffect* CostGE = GetCostGameplayEffect();
	if (CostGE)
	{
